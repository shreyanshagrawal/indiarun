...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const setupLenis = useCallback(async () => {
    try {
      ...
      if (useWindowScroll) {
        const lenis = new (Lenis as any)({ duration: 1.2, easing: (t: number) => Math.min(1, 1.001 - Math.pow(2, -10 * t)), smoothWheel: true, touchMultiplier: 2, lerp: 0.1 });
        ...
        const scroller = scrollerRef.current;
        if (!scroller) return;
        const lenis = new (Lenis as any)({ wrapper: scroller, content: scroller.querySelector('.scroll-stack-inner') as HTMLElement, duration: 1.2, easing: (t: number) => Math.min(1, 1.001 - Math.pow(2, -10 * t)), smoothWheel: true, touchMultiplier: 2, lerp: 0.1 });
        lenis.on('scroll', handleScroll);
        const raf = (time: number) => { lenis.raf(time); animationFrameRef.current = requestAnimationFrame(raf); };
        animationFrameRef.current = requestAnimationFrame(raf);
        lenisRef.current = lenis;
      ...
      // fallback: use native scroll events
  ...
  return (
    <div
      ...
      ref={scrollerRef}
      style={{ overscrollBehavior: 'contain', WebkitOverflowScrolling: 'touch' as any, transform: 'translateZ(0)', willChange: 'scroll-position' }}
    >
      <div className="scroll-stack-inner pt-[20vh] px-8 md:px-16 pb-[50rem] min-h-screen">
        {children}
